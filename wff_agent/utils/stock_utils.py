...
from ..datasource.alpha_v_utils import get_us_stock_info
...
def get_market_indicators(symbol: str, market: str, windows_size:int=50) -> Dict[str, Any]:
    ...
    df = akutils.get_stock_history(symbol, market)
    if df is None:
...
def get_latest_stock_price(symbol: str, market: str) -> Dict[str, Any]:
    ...
    if market == "cn":
        return akutils.get_cn_stock_info(symbol)
    elif market == "us":
...
def _get_cn_stock_news(symbol: str) -> List[Dict[str, Any]]:
    ...
    try:
        # 获取新闻数据
        df = akutils.get_cn_news_from_eastmoney(symbol).head(20) # 获取最新的5条新闻
        news_list = []
...
def _get_global_financial_news() -> List[Dict[str, Any]]:
    ...
    df = akutils.get_global_financial_news()
    if df is None:
